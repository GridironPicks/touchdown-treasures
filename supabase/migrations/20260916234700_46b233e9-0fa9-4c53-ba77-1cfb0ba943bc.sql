CREATE OR REPLACE FUNCTION public.picks_deadline(_season integer, _week integer, _season_type season_type DEFAULT 'reg'::season_type)
...
AS $function$
  SELECT LEAST(
    (date_trunc('day', min(g.kickoff) AT TIME ZONE 'America/Chicago')
       + interval '12 hours') AT TIME ZONE 'America/Chicago',
...
BEGIN
  ...
  IF _row.season_type = 'reg' THEN
    ...
    _open := public.picks_open_at(_row.season, _row.week, _row.season_type);

    IF _open IS NOT NULL AND now() < _open THEN
      RAISE EXCEPTION 'Week % is not open yet — picks open Monday 12:00 AM CT', _row.week;
    END IF;

    IF _deadline IS NOT NULL AND now() >= _deadline THEN
      RAISE EXCEPTION 'Picks for week % are locked (12:00 PM CT deadline on the first game day)', _row.week;
    END IF;
  ELSE
    SELECT count(*)::int,
